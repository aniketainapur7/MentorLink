...
const StudentDashboard: React.FC = () => {
  const { fetchStudentRequests, fetchStudentSessions } = useStudentSessionStore()
  const { sessions } = useSessionStore()

  // Example mock data for line chart
  const progressData = [
    { date: "Mon", hours: 2 },
    { date: "Tue", hours: 1.5 },
    { date: "Wed", hours: 3 },
  ...
  // Heatmap mock data (replace with real learning activity)
  const today = new Date()
  const heatmapData = Array.from({ length: 120 }).map((_, i) => {
    const date = new Date()
      ...
      >
        ...
        <CalendarHeatmap
          ...
          tooltipDataAttrs={(value) => {
            return {
              "data-tip": value.date ? `${value.date}: ${value.count} hrs` : "",
            }
          }}
        />
      </motion.div>
